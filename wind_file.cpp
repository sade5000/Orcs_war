...
WindAndRend::WindAndRend(const char* title , int p_width , int p_heigth)
{
    ...
    if(main_render == NULL)
    {
        std::cout<<INIT_RENDERER<<SDL_GetError()<<std::endl;
    }
}
...
SDL_Texture* WindAndRend::load_bkg_texture(const char* p_texture)
{
    SDL_Texture* texture = nullptr;
    texture = IMG_LoadTexture(main_render,p_texture);
    if(texture == NULL)
...
void WindAndRend::render_hdr(SDL_Renderer* render ,SDL_Texture* texture, int ax , int ay , int aw , int ah, int bx , int by , int bw , int bh)
{
    SDL_Rect src;
    src.x = ax;
    src.y = ay;
